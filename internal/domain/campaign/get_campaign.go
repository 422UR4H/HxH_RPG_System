...
type IGetCampaign interface {
	GetCampaign(uuid uuid.UUID) (*campaign.Campaign, error)
}
...
func (uc *GetCampaignUC) GetCampaign(uuid uuid.UUID) (*campaign.Campaign, error) {
	campaign, err := uc.repo.GetCampaign(context.Background(), uuid)
	if err != nil {
